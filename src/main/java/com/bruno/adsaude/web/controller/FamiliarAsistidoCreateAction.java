...
public class FamiliarAsistidoCreateAction extends Action{

	private static Logger logger = LogManager.getLogger(FamiliarAsistidoCreateAction.class);
	private AsistidoFamiliarService asistidoFamiliarService=null;
	
	public FamiliarAsistidoCreateAction() {
		super(ActionNames.FAMILIAR_ASISTIDO_CREATE);
...
public final String doIt(HttpServletRequest request, HttpServletResponse response) {
		...
		af.setIdAsistido(usuario.getId());
		af.setIdFamiliar(idFamiliar);
			try {
				if(idFamiliar!=null) {
